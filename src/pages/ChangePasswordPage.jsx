...
export default function ChangePasswordPage() {
  ...
  const urlToken       = searchParams.get('token'); // one-time token from email link
  ...
  // ── NEW: token exchange on mount ────────────────────────────────────────────
  // If the user arrived via the email link (?token=...), we call TokenLoginView
  // to exchange it for a real access token and store it in localStorage.
  // After that, ChangePasswordView (IsAuthenticated) will accept the Bearer token.
  const [tokenReady, setTokenReady] = useState(!urlToken); // true immediately if no URL token

  useEffect(() => {
    if (!urlToken) return; // already logged in, nothing to exchange

    axios.post(`${API_BASE_URL}/api/auth/token-login/`, { token: urlToken })
      .then(({ data }) => {
        localStorage.setItem('access_token',        data.access);
        localStorage.setItem('must_change_password', 'true');
        setTokenReady(true); // unlock the form
      })
      .catch(() => {
        setError('This link is invalid or has already been used. Please contact support.');
        setTokenReady(false);
      });
  }, [urlToken]);
  // ────────────────────────────────────────────────────────────────────────────
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // ChangePasswordView requires IsAuthenticated — Bearer token set above ──
      await axios.post(
    ...
    <>
      ...
      <div style={{
        ...
        >
          ...
          {/* ── Form — hidden until token is ready ── */}
          {tokenReady && (
          ...
          {/* ── Error when token is invalid (form stays hidden) ── */}
          {!tokenReady && error && (
