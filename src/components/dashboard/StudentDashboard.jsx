...
// ── Top Header ────────────────────────────────────────────────────────────────
function TopHeader({ user, activeSection, courseData, isMobile, onMenuOpen }) {
  ...
  return (
    <header style={{
      ...
      <div style={{
        ...
        gap: '8px', minWidth: 0, overflow: 'hidden',
      }}>
        <span style={{
      ...
      <div style={{
        display: 'flex', alignItems: 'center',
        gap: '10px', flexShrink: 0, marginLeft: '12px',
      }}>
        <div style={{
  ...
  return (
    ...
    >
      ...
      <p style={{
        ...
        color: THEME.textLight, lineHeight: 1.6, maxWidth: '500px',
      }}>
  ...
  return (
    ...
    >
      <div style={{ flex: 1, minWidth: '140px' }}>
        <p style={{ margin: '0 0 2px', fontSize: '12px', fontWeight: '700', color: THEME.text }}>
          Course Progress
        </p>
        <p style={{ margin: 0, fontSize: '11px', color: THEME.textLight }}>
          {pct < 100
            ? `${Math.round(100 - pct)}% remaining — you're doing great!`
            : 'Course complete 🎉'}
        </p>
      </div>
        ...
        <div style={{
          ...
          <motion.div
            ...
            style={{
              ...
              background: `linear-gradient(to right, ${THEME.primary}, #f59e0b)`,
            }}
          />
        </div>
      </div>
  ...
  useEffect(() => {
    ...
    const fetchCourseData = async () => {
      try {
        setLoading(true);
        setError(null);
        const response = await api.get('/api/dashboard/my-course/', {
          signal: abortController.signal,
        });
        if (isMounted) setCourseData(response.data);
      } catch (err) {
    ...
    const fetchSectionData = async () => {
      try {
        ...
        if (activeSection === STUDENT_SECTIONS.PROGRESS) {
          const response = await api.get('/api/dashboard/my-progress/', {
            signal: abortController.signal,
          });
        ...
        if (activeSection === STUDENT_SECTIONS.CERTIFICATE) {
          try {
            const response = await api.get('/api/dashboard/my-certificate/', {
              signal: abortController.signal,
            });
        ...
        if (err.name !== 'AbortError' && isMounted) {
          if (import.meta.env.DEV) console.debug('[StudentDashboard] Section fetch failed:', err.message);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 767px) {
          .sdash-topbar-padding { padding-left: 64px !important; }
        }
      `}</style>
